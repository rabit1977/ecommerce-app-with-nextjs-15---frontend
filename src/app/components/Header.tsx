...
const Header = () => {
    ...
    // 👇 UPDATED THIS ARRAY TO INCLUDE ALL 10 COLORS
    const themes = [
                        ...
                        {showThemes && (
                            <div className="absolute top-full right-0 mt-2 w-32 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-2 z-30">
                                <div className="grid grid-cols-4 gap-2">
                                    {themes.map(t => (
                                        <button key={t.name} onClick={() => { setThemeScheme(t.name); setShowThemes(false); }} className="w-full aspect-square rounded-full" style={{ backgroundColor: t.color }}></button>
                                    ))}
                                </div>
                            </div>
                        )}
